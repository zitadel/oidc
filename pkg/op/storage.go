...
// BackchannelAuthenticationStorage is an optional interface for CIBA (Client Initiated Backchannel Authentication) support.
// Implementers of this interface enable the backchannel authentication endpoint and CIBA grant type.
type BackchannelAuthenticationStorage interface {
	// StoreBackchannelAuthentication stores a new CIBA authentication request.
	// The auth_req_id should be generated by the caller with sufficient entropy (recommended: 128-bit).
	// The implementer should trigger notification to the user's authentication device after storing the request.
	StoreBackchannelAuthentication(ctx context.Context, clientID, authReqID string, expires time.Time, scopes []string, loginHint string, bindingMessage string) error

	// GetBackchannelAuthenticationState returns the current state of the CIBA authentication flow.
	// This method is polled by the client until the authentication is completed, denied, or expired.
	// If the context deadline is exceeded, implementations should return ctx.Err() to trigger a slow_down response.
	GetBackchannelAuthenticationState(ctx context.Context, clientID, authReqID string) (*BackchannelAuthenticationState, error)

	// CompleteBackchannelAuthentication marks the authentication request as approved by the user.
	// This is typically called by the authentication device after the user successfully authenticates and approves the request.
	CompleteBackchannelAuthentication(ctx context.Context, authReqID, subject string, amr []string, authTime time.Time) error

	// DenyBackchannelAuthentication marks the authentication request as denied by the user.
	// This is called when the user authenticates but explicitly denies the authorization request.
	DenyBackchannelAuthentication(ctx context.Context, authReqID string) error
}

func assertBackchannelStorage(s Storage) (BackchannelAuthenticationStorage, error) {
	storage, ok := s.(BackchannelAuthenticationStorage)
	if !ok {
		return nil, oidc.ErrUnsupportedGrantType().WithDescription("urn:openid:params:grant-type:ciba not supported")
	}
	return storage, nil
}
